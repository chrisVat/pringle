"""
pregglenator v18 — modulo machine assignment + two-level worker SA

Changes from v17
----------------
* Machine assignment is now simple modulo hash (u % num_machines).
  This matches Pregel's default random hash behaviour at the machine level,
  giving statistically even machine loads without any graph-aware partitioning.
* All machine-level METIS and SA code removed.
* Worker assignment remains METIS initialisation + SA refinement within each
  machine (unchanged from v17).
* Removed --sa_iters and --t_start CLI args (machine SA no longer exists).

Rationale
---------
Previous experiments showed machine-level comm-aware partitioning reduced
total messages (~35%) but increased query time (~2x) due to load imbalance
across machines (machine 3 doing 3x the work of machine 1).

Modulo hash distributes vertices evenly across machines by construction.
Worker-level SA then optimises intra-machine worker assignment against:
    c_proc × cross-worker intra-machine traffic
  + c_node × Σ_s  max_worker_in_machine(recv-load in superstep s)

Algorithm
---------
1. Load traces → undirected graph
...
Output JSON (same schema as v17)
...
def load_comm_traces(traces_dir):
    ...
    for path in tqdm(paths, desc="  Reading traces", unit="file"):
        with open(path, newline="") as fh:
            for row in tqdm(csv.DictReader(fh),
                            desc=f"    {os.path.basename(os.path.dirname(path))}",
...
def sa_optimize(
    ...
    """
    Generic SA partition refiner — used at the worker level.
...
def assign_workers_sa(
    ...
    Set worker_sa_iters=0 to skip SA and use METIS only.
    """
    n         = len(und_adj)
    worker_of = [-1] * n
    wpm       = []
...
def compute_worker_balance(
    ...
    For each machine returns:
      sum_max_worker_load  — Σ_s max_worker(recv-load in s)
...
def main():
    ap = argparse.ArgumentParser(
        description="Superstep-aware Pregel partitioner v18 — modulo machine + worker SA"
    )
    ...
    ap.add_argument("--c_net",             type=float, default=500.0,
                    help="Cost weight for cross-machine messages (default 500) "
                         "— used only in stats reporting, not in SA optimisation")
    ap.add_argument("--c_proc",            type=float, default=1.0,
                    help="Cost weight for cross-worker intra-machine messages (default 1)")
    ...
    ap.add_argument("--output",            default='partition_v18_modulo.json',
    ...
    if args.nodes_per_machine < 0:
        ...
        print(f"  nodes_per_machine = {args.nodes_per_machine} "
              f"(auto: ceil({n}/{args.num_machines}))")
    if args.nodes_per_worker < 0:
        args.nodes_per_worker = math.ceil(args.nodes_per_machine / 4)
        print(f"  nodes_per_worker  = {args.nodes_per_worker} "
              f"(auto: ceil({args.nodes_per_machine}/4), ~4 workers/machine)")
    ...
    print(f"  Machine cut weight (informational): {machine_cut:,.1f}")
    ...
    if args.worker_sa_iters > 0:
        print(f"Assigning workers within machines "
              f"(METIS + SA, {args.worker_sa_iters:,} iters/machine)...")
    else:
    ...
    print(f"  machine_cut_weight        : {machine_cut:,.1f}")
