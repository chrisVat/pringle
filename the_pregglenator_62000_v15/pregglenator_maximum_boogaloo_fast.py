...
def _load_single_trace_combiner_aware(path):
    ...
    if _HAVE_PANDAS:
        ...
        for (s, v), csum in tqdm(
            ...
            raw_recv_local[int(v)][(task_name, int(s))] += float(csum)

        presence_df = grouped.reset_index()[["superstep", "src_vertex", "dst_vertex"]]

        # Reuse tuple objects for step keys to reduce Python allocation churn
        unique_steps = presence_df["superstep"].unique()
...
def load_comm_traces_combiner_aware(traces_dir, max_traces=-1, trace_paths=None):
    ...
    for path in tqdm(paths, desc="Reading traces", unit="file"):
        task_name, out_local, recv_local, pair_local, step_keys_local = _load_single_trace_combiner_aware(path)
...
def compute_combiner_aware_final_comm_cost(
    ...
    net_cost = 0.0
    for (_, _), hist in machine_hist.items():
...
def main():
    ...
    ap.add_argument("--c_proc", type=float, default=1e-9,
    ...
    ap.add_argument("--output", default="pregglenator_maximum.json",
    ...
    stats = {
        "version": "pregglenator_v18_combiner_aware_fastload",
