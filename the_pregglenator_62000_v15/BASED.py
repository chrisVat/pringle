"""
BASED: modulo machine assignment + two-level worker SA
...
Worker-level SA then optimises intra-machine worker assignment against:
    c_proc x cross-worker intra-machine traffic
  + c_node x Σ_s  max_worker_in_machine(recv-load in superstep s)
...
def _sample_temperature(
    ...
    """Estimate T_start as 2x mean |delta cost| over k random moves."""
    deltas = []
    for _ in range(k):
        v      = rng.randrange(n)
        p_from = part_of[v]
...
def sa_optimize(
    ...
    Minimises:
      c_comm x Σ cross-partition message counts (aggregated across supersteps)
    + c_node x Σ_s  max_part( recv-load on partition in superstep s )
    """
