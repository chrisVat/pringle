...
def build_graph_superstep_weights(comm_by_superstep, total_comm, n):
    ...
    Edges active in dense supersteps accumulate more weight than edges only
    seen in sparse supersteps.  All weights are in (0, num_supersteps].
    """
...
def _build_pymetis_inputs(und_adj):
    ...
    for u in range(len(und_adj)):
        for v, w in sorted(und_adj[u].items()):
            adjncy.append(v)
            eweights.append(max(1, int(round(w * 1000))))  # scale up since weights are small floats
...
def build_vertex_edge_index(comm_by_superstep, n):
    ...
    Always uses raw counts (independent of graph weights) because these
    feed the SA load-balance calculation, not METIS.
    """
...
def main():
    ap = argparse.ArgumentParser(
        description="Superstep-aware Pregel partitioner v20 — modulo machine + worker SA + superstep-density weights"
    )
    ...
    ap.add_argument("--worker_sa_iters",   type=int,   default=500_000,
                    help="Worker-level SA iterations per machine. Set to 0 for METIS only.")
    ap.add_argument("--t_end",             type=float, default=1e-4)
    ap.add_argument("--seed",              type=int,   default=42)
    ap.add_argument("--output",            default='partition_v20.json')
    args = ap.parse_args()
